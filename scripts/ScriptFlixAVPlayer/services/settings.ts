import type { ApiSource, DanmuBindingMap, DanmuConfig } from '../types'
...
const STORAGE_KEY_DANMU_CONFIG = 'script_flix_danmu_config'
const STORAGE_KEY_DANMU_BINDINGS = 'script_flix_danmu_bindings'

const defaultSources: ApiSource[] = []
const defaultDanmuConfig: DanmuConfig = {
  enabled: false,
  baseUrl: '',
  opacity: 0.9,
  fontSize: 16,
...
export const SettingsService = {
  ...
  getDanmuConfig: (): DanmuConfig => {
    const raw = AppStorage.get(STORAGE_KEY_DANMU_CONFIG)
    if (!raw) return defaultDanmuConfig
    return { ...defaultDanmuConfig, ...(raw as Partial<DanmuConfig>), modes: { ...defaultDanmuConfig.modes, ...((raw as any)?.modes || {}) } }
  },

  saveDanmuConfig: (config: DanmuConfig) => {
    AppStorage.set(STORAGE_KEY_DANMU_CONFIG, config)
  },

  getDanmuBindings: (): DanmuBindingMap => {
    return (AppStorage.get(STORAGE_KEY_DANMU_BINDINGS) || {}) as DanmuBindingMap
  },

  saveDanmuBinding: (key: string, value: DanmuBindingMap[string]) => {
    const next = { ...SettingsService.getDanmuBindings(), [key]: value }
    AppStorage.set(STORAGE_KEY_DANMU_BINDINGS, next)
  },

  removeDanmuBinding: (key: string) => {
    const prev = SettingsService.getDanmuBindings()
    const next = Object.fromEntries(Object.entries(prev).filter(([k]) => k !== key)) as DanmuBindingMap
    AppStorage.set(STORAGE_KEY_DANMU_BINDINGS, next)
  }
}
