...
export type DanmuPositionMode = 'scroll' | 'top' | 'bottom'
...
export type DanmuConfig = {
  ...
  opacity: number
  fontSize: number
  modes: Record<DanmuPositionMode, boolean>
}

export type DanmuMatchResult = {
  commentId: string
  title: string
  episode?: string
  source?: string
...
export type DanmuBinding = {
  commentId: string
  title: string
  boundAt: number
}

export type DanmuBindingMap = Record<string, DanmuBinding>
...
export type DanmuBindScreenProps = {
  baseUrl: string
  videoTitle: string
  episodeName?: string
  defaultQuery: string
  bindingKey: string
  onBound: (binding: DanmuBinding) => void
}
